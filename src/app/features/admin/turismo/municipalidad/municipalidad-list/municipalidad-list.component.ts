  ...
  template: `
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Gestión de Municipalidad</h1>
      ...
      <div class="rounded-lg bg-white shadow-sm overflow-hidden">
        @if (loading) {
          ...
          <div class="p-8 text-center">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">No se encontraron municipalidades</h3>
            <p class="mt-1 text-sm text-gray-500">Comienza creando una nueva municipalidad.</p>
            <div class="mt-6">
              <a routerLink="/admin/municipalidad/create" class="inline-flex items-center rounded-md bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
                <svg class="-ml-1 mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                </svg>
                Nueva Municipalidad
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Redes Sociales</th>
                ...
                @for (municipalidad of municipalidades; track municipalidad.id) {
                  <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm font-medium text-gray-900">{{ municipalidad.nombre }}</div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="text-sm text-gray-500 truncate max-w-xs">{{ municipalidad.descripcion }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-500">{{ municipalidad.correo || 'No disponible' }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex space-x-2">
                        @if (municipalidad.red_facebook) {
                          <a [href]="municipalidad.red_facebook" target="_blank" class="text-blue-600 hover:text-blue-900">
                        ...
                        @if (municipalidad.red_instagram) {
                          <a [href]="municipalidad.red_instagram" target="_blank" class="text-pink-600 hover:text-pink-900">
                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        @if (municipalidad.red_youtube) {
                          <a [href]="municipalidad.red_youtube" target="_blank" class="text-red-600 hover:text-red-900">
                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
