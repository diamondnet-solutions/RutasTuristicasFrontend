      ...
      <div class="rounded-lg bg-white p-6 shadow-sm">
        @if (loading) {
          ...
          <div class="flex justify-between items-center mb-6">
            <button 
              (click)="mesAnterior()"
              class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-1 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
            >
            ...
            <h2 class="text-xl font-semibold text-gray-900">{{ nombreMes }} {{ anioActual }}</h2>
          ...
          <div class="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden">
            ...
            @for (dia of diasSemana; track dia) {
              <div class="bg-gray-100 p-2 text-center text-sm font-medium text-gray-700">
                {{ dia }}
              </div>
            }
            ...
            @for (celda of celdasCalendario; track celda.fecha) {
              <div
                class="bg-white min-h-[100px] p-2 {{ celda.esFueraDeMes ? 'bg-gray-50 opacity-50' : '' }}"
              >
                <!-- Número del día -->
                <div class="flex justify-between items-start">
                  <span class="font-medium text-gray-700 {{ esHoy(celda.fecha) ? 'p-1 bg-primary-100 text-primary-800 rounded-full' : '' }}">
                ...
                <div class="mt-1 space-y-1 max-h-[120px] overflow-auto">
                  @for (reserva of celda.reservas; track reserva.id) {
                    <a
                      ...
                      class="block px-2 py-1 rounded-md text-xs {{ getEstadoClass(reserva.estado) }}"
          ...
          <div class="mt-4 flex flex-wrap gap-3">
            <div class="flex items-center">
              <span class="inline-block w-3 h-3 mr-1 bg-yellow-100 border border-yellow-400 rounded-sm"></span>
              <span class="text-xs text-gray-700">Pendiente</span>
            </div>
            <div class="flex items-center">
              <span class="inline-block w-3 h-3 mr-1 bg-green-100 border border-green-400 rounded-sm"></span>
              <span class="text-xs text-gray-700">Confirmada</span>
            </div>
            <div class="flex items-center">
              <span class="inline-block w-3 h-3 mr-1 bg-red-100 border border-red-400 rounded-sm"></span>
              <span class="text-xs text-gray-700">Cancelada</span>
            </div>
            <div class="flex items-center">
              <span class="inline-block w-3 h-3 mr-1 bg-blue-100 border border-blue-400 rounded-sm"></span>
              <span class="text-xs text-gray-700">Completada</span>
            </div>
          </div>
        }
      </div>
...
export class ReservaCalendarioComponent implements OnInit {
  ...
  getEstadoClass(estado: string): string {
    switch (estado) {
      case 'pendiente': return 'bg-yellow-100 text-yellow-800 border-l-2 border-yellow-400';
      case 'confirmado': return 'bg-green-100 text-green-800 border-l-2 border-green-400';
      case 'cancelado': return 'bg-red-100 text-red-800 border-l-2 border-red-400';
      case 'completado': return 'bg-blue-100 text-blue-800 border-l-2 border-blue-400';
      default: return 'bg-gray-100 text-gray-800 border-l-2 border-gray-400';
    }
  }
